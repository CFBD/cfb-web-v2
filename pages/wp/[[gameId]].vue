...
const title = ref("");
const gameId: Ref<number | undefined> = ref(route.params.gameId ? Number(route.params.gameId) : undefined);
const plays: Ref<{ home: string, homeId: number, homeScore: number, away: string, awayId: number, awayScore: number, playNumber: number, playText: string, homeWinProbability: number }[]> = ref([]);
...
const loadChartData = () => {
    if (gameId.value) {
        $fetch<{ home: string, homeId: number, homeScore: number, away: string, awayId: number, awayScore: number, playNumber: number, playText: string, homeWinProbability: number }[]>('/metrics/wp', {
            method: 'GET',
