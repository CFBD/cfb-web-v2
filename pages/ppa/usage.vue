<template>
  <Card class="text-center m-3">
    <template #title> Player Efficiency </template>
    <template #subtitle> PPA and Usage </template>
    <template #content>
...
const positions = ref(['QB', 'RB', 'WR', 'TE', 'FB']);
...
const reloadData = () => {
  ...
  $fetch('/player/usage', {
    ...
    $fetch('/ppa/players/season', {
      ...
      const playerData = usageData
        .map(
          (r: {
            ...
            let team = mainStore.teams.find(
              (t) => t.classification == 'fbs' && t.school == r.team,
            );
      ...
      tooltips.value = playerData.map(
        (p: { name: string; team: { school: string } }) =>
          `${p.name} (${p.team.school})`,
      );
        ...
        datasets: [
          {
            ...
            pointRadius: 5,
            pointBorderWidth: 3,
            pointBorderColor: playerData.map(
              (r: { team: { color: string } }) => r.team.color,
            ),
            pointBackgroundColor: playerData.map(
              (r: { team: { alt_color: string } }) => r.team.alt_color,
            ),
