<template>
  <Card class="text-center m-3">
    <template #title> SP+ Team Trends </template>
    <template #subtitle>
      <p>
        SP+ is a rating system created by Bill Connelly (<a
          href="https://twitter.com/ESPN_BillC"
          >@ESPN_BillC</a
        >) that are meant to be a predictive comparison of teams. For a more
      ...
      <p>
        Select a team and a data points to begin generating trend
        visualizations.
      </p>
  ...
  if (selectedTeam.value) {
    $fetch('/ratings/sp', {
      ...
      $fetch('/ratings/sp/conferences', {
        ...
        baseURL: config.public.apiBaseUrl,
        params: {
          conference: mainStore.conferences.find(
            (c) => c.name == selectedTeam.value?.conference,
          )?.abbreviation,
        },
      }).then((result) => {
...
const reloadData = () => {
  if (selectedTeam.value && selectedOption.value && teamData.value) {
    ...
    let data = teamData.value
      .filter((r: { team: string }) => r.team == selectedTeam.value.school)
      .map((r: { year: number }) => ({
        x: r.year,
        y: getValueByKey(r, selectedOption.value.split('.')),
      }))
      .filter((r: { x: number; y: number }) => r.y);
    let averages = teamData.value
      .filter((r: { team: string }) => r.team == 'nationalAverages')
      .map((r: { year: number }) => ({
        x: r.year,
        y: getValueByKey(r, selectedOption.value.split('.')),
      }))
      .filter((r: { x: number; y: number }) => r.y);
    let conferenceAverages = data
      .map((l: { x: number; y: number }) => {
        let item = conferenceData.value.find(
          (d: { year: number }) => d.year == l.x,
        );
        return item
          ? { x: l.x, y: getValueByKey(item, selectedOption.value.split('.')) }
          : null;
      })
    ...
    if (
      ...
      comparisonTeamData.value &&
      comparisonTeamData.value.length
    ) {
      datasets.push({
        pointRadius: 5,
        ...
        data: comparisonTeamData.value
          .filter(
            (r: { team: string }) => r.team == comparisonTeam.value.school,
          )
          .map((r: { year: number }) => ({
            x: r.year,
            y: getValueByKey(r, selectedOption.value.split('.')),
    ...
    if (
      ...
      (comparisonTeam.value &&
        selectedTeam.value.conference == comparisonTeam.value.conference)
    ) {
      datasets.push({
        borderDash: [10, 5],
        pointRadius: 0,
