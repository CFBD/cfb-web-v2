<template>
  <Card class="text-center m-3">
    ...
    <template #subtitle> Select a team </template>
    <template #content>
      <div class="grid">
        <div v-for="c in conferences" :key="c.name" class="col-6 md:col-4 mt-3">
          ...
          <div v-for="t in c.teams" :key="t.id" class="text-left">
            <Button @click="goto(t.name)" link
              ><img
                :src="`https://cdn.collegefootballdata.com/logos/48/${t.id}.png`"
              /><span class="ml-2">{{ t.name }}</span></Button
            >
          </div>
        </div>
      </div>
...
let conferences: Ref<
  { name: string; teams: { id: number; name: string }[] }[]
> = ref([]);

const loadTeams = () => {
  let confs = Array.from(
    new Set(
      mainStore.teams
