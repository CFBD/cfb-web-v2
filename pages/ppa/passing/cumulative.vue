<template>
  <Card class="text-center m-3">
    <template #title> QB Passing Trends </template>
    <template #subtitle>
...
const players: Ref<{ id: number; displayName: string; teamColor: string }[]> =
  ref([]);
...
const removePlayer = (player: {
  ...
  data.datasets = data.datasets.filter(
    (d: { label: string }) => d.label != player.displayName,
  );
