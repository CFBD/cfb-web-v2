<template>
  <Card class="text-center m-3">
    <template #title> Team Metrics </template>
    <template #subtitle> Select metric criteria to compare </template>
    <template #content>
      ...
      <div class="formgrid grid mt-3 justify-content-center">
        <div class="field col-12 sm:col-6 lg:col-3">
          ...
          <Dropdown
            ...
            optionLabel="text"
            optionValue="value"
            v-model="dataPoint1.playFilter"
            class="w-full"
            @change="reloadData"
          ></Dropdown>
          <Dropdown
            :options="metricTypes1"
            optionLabel="text"
            optionValue="value"
            v-model="dataPoint1.metricType"
        ...
        <div class="field col-12 sm:col-6 lg:col-3">
          ...
          <Dropdown
            ...
            optionLabel="text"
            optionValue="value"
            v-model="dataPoint2.playFilter"
            class="w-full"
            @change="reloadData"
          ></Dropdown>
          <Dropdown
            :options="metricTypes2"
            optionLabel="text"
            optionValue="value"
            v-model="dataPoint2.metricType"
            placeholder="Select a metric"
            class="w-full"
            @change="reloadData"
          ></Dropdown>
...
const metricTypes1 = computed(() =>
  metricTypes.value.filter((m) =>
    m.playFilters.includes(dataPoint1.value.playFilter),
  ),
);
const metricTypes2 = computed(() =>
  metricTypes.value.filter((m) =>
    m.playFilters.includes(dataPoint2.value.playFilter),
  ),
);
...
const reloadData = () => {
  if (
    dataItems.value &&
    dataPoint1.value.statType &&
    dataPoint1.value.metricType &&
    dataPoint2.value.statType &&
    dataPoint2.value.metricType
  ) {
    // this.$ga.event('visualization', 'generation', 'season-metrics');

    // @ts-ignore
    let points = dataItems.value
      .filter(
        (r) => conference.value === 'All' || r.conference == conference.value,
      )
      .map((r) => {
        let img = new Image();
      ...
      datasets: [
        {
          ...
          data: dataItems.value
            .filter(
              (r) =>
                conference.value == 'All' || r.conference == conference.value,
            )
            .map((r) => ({
