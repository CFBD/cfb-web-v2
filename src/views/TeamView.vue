...
import type { PlayerPPA, PlayerUsage, TeamStats } from '@/types/teamStatTypes';
...
onBeforeMount(async () => {
    if (router.currentRoute.value.params.team) {
        // eslint-disable-next-line
        // @ts-ignore
        team.value = router.currentRoute.value.params.team;
        loadData();
    }
});
...
onBeforeRouteUpdate((to) => {
    // eslint-disable-next-line
    // @ts-ignore
    if (to.name === "team" && to.params.team && to.params.team != team.value) {
        // eslint-disable-next-line
        // @ts-ignore
        team.value = to.params.team;
        loadData();
    }
});
</script>
